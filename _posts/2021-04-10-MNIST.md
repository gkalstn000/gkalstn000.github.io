...
# GPU로 바꾸려했지만, RuntimeError: Tensor for argument #2 ‘weight’ is on CPU, but expected it to be on GPU (while checking arguments for cudnn_batch_norm) 가 떠서 해결법을 못찾음
...
```python
model, opt = get_model(layer = 1, lr = 0.05)
train_loss = fit(epochs, model, loss_func, opt, train_dl, valid_dl)
```

    train accuracy :  tensor(0.9058)
    vaild accuracy :  tensor(0.9121)
...
```python
model, opt = get_model(layer = 2, lr = 0.05)
train_loss = fit(epochs, model, loss_func, opt, train_dl, valid_dl)
```

    train accuracy :  tensor(0.9180)
    vaild accuracy :  tensor(0.9185)
...
```python
model, opt = get_model(layer = 3, lr = 0.05)
train_loss = fit(epochs, model, loss_func, opt, train_dl, valid_dl)
```

    train accuracy :  tensor(0.9204)
    vaild accuracy :  tensor(0.9201)
...
```python
model, opt = get_model(layer = 4, lr = 0.05)
train_loss = fit(epochs, model, loss_func, opt, train_dl, valid_dl)
```

    train accuracy :  tensor(0.9203)
    vaild accuracy :  tensor(0.9188)
...
```python
model, opt = get_model(layer = 5, lr = 0.05)
train_loss = fit(epochs, model, loss_func, opt, train_dl, valid_dl)
```

    train accuracy :  tensor(0.9192)
    vaild accuracy :  tensor(0.9185)
...
# 종합적인 비교
> 각 layer 노드가 많아 시간이 오래걸려 하이퍼파라미터 튜닝을 많이 못해봤지만, 대체적으로 **layer가 많다고 정확도가 좋아지진 않았다.** 
>
> 확실이 Machine Learning 모델(SVM, Logistic Regression 등등)로 돌렸을 때보다 성능이 많이 좋은것이 체감이되고, torch.nn 모듈 사용으로 인해 코드가 정말 간결해졌다.  


```python
train_accuracy = [0.9058, 0.9180, 0.9204, 0.9203, 0.9192]
valid_accuracy = [0.9121, 0.9185, 0.9201, 0.9188, 0.9185]
x = [1, 2, 3, 4, 5]

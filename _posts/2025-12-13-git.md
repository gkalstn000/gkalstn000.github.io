...
`reset`, `revert` 둘다 commit 한거 되돌리는 작업인데 우선 `reset` 부터 보자.  

### 1.3.1 Reset

`reset`을 이해하려면 git이 관리하는 3가지공간 The Three Trees (Git의 3단 구조)을 이해해야한다.  
Git은 파일을 단순히 '저장'하는 게 아니라, **3단계의 버퍼**를 거쳐 이동시킴. `reset`의 옵션(`--soft, --mixed, --hard`)은 이 3단계 중 어디까지 되돌릴 것인가를 결정하는 스위치다.

| 영역 (Area)              | 역할 (Role)                         | 물리적 실체 (Physical)                     |
|------------------------|-----------------------------------|---------------------------------------|
| HEAD                   | 현재 브랜치의 끝 (Last Commit)을 가리키는 포인터 | `.git/HEAD` -> `.git/refs/heads/main` |
| Index (Staging Area)   | 다음 커밋이 될 파일들의 스냅샷(예비 저장소).        | `.git/index` (Binary File)            |
| Working Directory      | 눈으로 보고 수정하는 실제 파일들 (Sand Box).    | 프로젝트 폴더 그 자체         |
